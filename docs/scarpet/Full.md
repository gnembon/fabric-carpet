...
use them as intergers, you can. Strings use single quoting, for multiple reasons, but primarily to allow for easier use of strings inside doubly quoted command arguments (when passing a script as a parameter of /script fill for example), or when typing in jsons inside scarpet (to feed back into a /data merge command for example). Strings also use backslashes \ for quoting special characters, in both plain strings and regular expressions
