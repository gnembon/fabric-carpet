    ...
    public static void register(CommandDispatcher<ServerCommandSource> dispatcher)
    {
        ...
        LiteralArgumentBuilder<ServerCommandSource> a = literal("load").requires( (player) -> SettingsManager.canUseCommand(player, CarpetSettings.commandScriptACE) ).
                then(argument("app", StringArgumentType.word()).
                        ...
                        executes((cc) ->
                        {
                            boolean success = CarpetServer.scriptServer.addScriptHost(cc.getSource(), StringArgumentType.getString(cc, "app"), true, false, null);
                            return success?1:0;
                        }).
                        then(literal("global").
                                executes((cc) ->
                                {
                                    boolean success = CarpetServer.scriptServer.addScriptHost(cc.getSource(), StringArgumentType.getString(cc, "app"), false, false, null);
                                    return success?1:0;
                                }
                                )
                        )
