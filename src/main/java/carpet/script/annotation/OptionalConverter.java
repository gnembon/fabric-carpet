 ...
 * 
 * <p>Note that it will not work properly if it's not at the end of the function, since values are
 * consumed in an ordered way, therefore it will throw if the value is not present since it will try
 * to evaluate the next parameter into {@code <R>} and horribly fail, or it will draw values from the
 ...
 * <li>An incorrect value ({@code null}), if the value was present but incorrect, as per the contract of {@link ValueConverter}
 *     and the way {@link AnnotationParser} will consider {@link null} values</li>
 * <li>An empty object, if the value was not present (or {@code null})</li>
 * </ul>
 *
...
final class OptionalConverter<R> implements ValueConverter<Optional<R>> {
	...
	/**
	 * <p>Returns a new {@link OptionalConverter} to convert to the type in the given {@link AnnotatedType} if there is at least one element
	 * left in the iterator, with any parameters, annotations or anything that was present in there.</p>
	 * 
	 * <p>The given {@link ValueConverter} will be called after ensuring that there is at least one element left in the iterator to the type
	 * specified in the generics of this {@link AnnotatedType}, or return an empty {@link Optional} if there is nothing left or the first value
	 * meets {@link Value#isNull()}.
	 * 
	 * @apiNote This method expects the {@link AnnotatedType} to already be of {@link Optional} type, and, while it will
	 *          technically accept a non-{@link Optional} {@link AnnotatedType}, it will fail with an {@link ArrayIndexOutOfBoundsException}
