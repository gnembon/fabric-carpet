...
/**
 * <p>A converter from a given {@link Object} of type T into a {@link LazyValue}, used in order to convert the outputs
 * of methods into usable Scarpet values.</p>
 *
...
public final class OutputConverter<T> {
	...
	public static <T> OutputConverter<T> get(Class<T> returnType) {
		...
		return (OutputConverter<T>)Objects.requireNonNull(byResult.get(returnType), "Unregistered output type: "+returnType+". Register in OutputConverter");
	}
	...
	public static <T> void register(Class<T> inputType, Function<T, LazyValue> converter) {
		...
		if (byResult.containsKey(inputType)) throw new IllegalArgumentException(inputType + " already has a registered OutputConverter");
		byResult.put(inputType, instance);
	}
	
	/**
	 * <p>Registers a new type to be able to be used as the return value of methods, converting from inputType to a {@link LazyValue},
	 * with the converter returning a {@link Value} type.</p>
	 * 
	 * @see #register(Class, Function)
	 * @param <T> The type of the input type
	 * @param inputType The class of T
